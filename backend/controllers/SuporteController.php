    ...
    public function index(): void
    {
        ...
        if ($this->isPost() && isset($_POST['resolver_chamado'])) {
            try {
                ...
                $this->flash('<div class="alert alert-danger alert-autohide mb-4">Erro ao encerrar chamado.</div>');
            }
            $this->redirect('suporte', [], 'sessao-chamados-pendentes');
        }
        ...
        if ($this->isPost()) {
            $params = [];
            $dataBusca = $_POST['data_busca'] ?? $_GET['data_busca'] ?? null;
            if ($dataBusca) {
                $params['data_busca'] = $dataBusca;
            }
            $this->finishPostRedirect('suporte', 'sessao-mapa-diario', $params, $mensagem);
        }
        ...
        $fotoAtual       = Auth::foto();
        $mapaEnsalamento = EnsalamentoHelper::mapaCentralizado($this->pdo);
